...
<?php if (!empty($update)): ?>
	<section class="update-banner" role="status" aria-live="polite" data-update-version="<?= $this->escape($update['current']) ?>">
		<div>
			<strong>QuickInstall <?= $this->escape($update['current']) ?> available</strong>
			<span>Download the latest release from phpBB.</span>
		</div>
		<div class="update-actions">
			<?php if (!empty($update['download'])): ?>
				<a class="button primary" href="<?= $this->escape($update['download']) ?>" target="_blank" rel="noreferrer">Download</a>
			<?php endif; ?>
...
<section class="status-strip" aria-label="Workspace summary">
	<?php foreach ($metrics as $metric): ?>
		<article class="metric">
			...
			<small><?= $this->escape($metric['detail']) ?></small>
			<em><?= $this->escape($metric['description']) ?></em>
		</article>
	<?php endforeach; ?>
</section>
...
<?php if ($notice !== '' || $error !== ''): ?>
	<section class="toast-stack" role="status" aria-live="polite">
		<?php if ($notice !== ''): ?>
			<p class="notice"><?= $this->escape($notice) ?></p>
		<?php endif; ?>
		<?php if ($error !== ''): ?>
...
<section class="section" id="create">
	...
	<form method="post" class="card settings-form" data-ajax>
		...
		<label class="field" title="Database engine for the board containers. SQLite supports the tiny and development seed presets."><span>DB</span><select name="db"><?php foreach ($dbOptions as $option): ?><option value="<?= $this->escape($option) ?>"><?= $this->escape($option) ?></option><?php endforeach; ?></select></label>
			...
			<table>
				...
				<tbody>
					<?php foreach ($sources as $source): ?>
						...
						?>
						<tr>
							<td><?= $this->escape($sourceKey) ?></td>
							<td><?= $this->escape($source['version'] ?? '') ?></td>
							<td><?= $this->escape($source['status'] ?? '-') ?></td>
							<td><?= !empty($source['downloaded']) ? 'yes' : 'no' ?></td>
							<td><?= $this->escape($usedBy ? implode(', ', $usedBy) : '-') ?></td>
							<td class="path-cell"><?= $this->escape($source['display_path'] ?? $source['path'] ?? '') ?></td>
							<td>
								<form method="post" class="source-remove" data-ajax data-confirm="<?= $this->escape($confirm) ?>">
									<?php require __DIR__ . '/csrf.php'; ?>
