...
class Application
{
	...
	private function renderSources(array $sources): void
	{
		...
		{
			echo '<div class="table-wrap"><table><thead><tr><th>Source</th><th>Version</th><th>Status</th><th>Downloaded</th><th>Used by</th><th>Path</th></tr></thead><tbody>';
			foreach ($sources as $source)
			{
				echo '<tr><td>' . $this->e($source['source_key'] ?? '') . '</td><td>' . $this->e($source['version'] ?? '') . '</td><td>' . $this->e($source['status'] ?? '-') . '</td><td>' . (!empty($source['downloaded']) ? 'yes' : 'no') . '</td><td>' . $this->e(implode(', ', $source['used_by'] ?? [])) . '</td><td>' . $this->e($source['path'] ?? '') . '</td></tr>';
			}
